...
```
my note: == is just for one, %in% allows for multiple filter
...
my note: 365 rows because each row in measurement has been pivoted wider.
...
```
my note: [] is to get into the dataframes, y can be response variable and predicted variable, while x is the explanatory variable and predictor variable
