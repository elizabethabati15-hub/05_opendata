...
```
Across the 24 hour, tmax_deg_c and tmin_deg_c was the maximum and minimum
...
```
pcp_mm_day and swe_kg_m_2has no pattern because the values are mostly 0 that is it's most likely not raining
...
Think of features as a new variable
...
fe_month
```
you could also do this by week using mutate(month = week(date))
...
Always watch out the number of your rows to see if it makes sense and what is expected
